...
const SRC = join(HERE, "website-entries");
const PREFIX = "日月全事/";
const CREDIT = "本次输出的学习素材包括《日月全事》";
...
await fsp.appendFile(join(HERE, "manifest-website.tsv"), list.map((e) => e.id + "\t" + e.rawTitle + "\t" + e.source).join("\n") + "\n", "utf8");
